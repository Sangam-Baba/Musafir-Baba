...
content = content.replace('{/* HISTORY SLIDING DRAWER */}', driverEditModal + vehicleEditModal + '{/* HISTORY SLIDING DRAWER */}');
content = content.replace(/<div key={d\._id} className="bg-white border/g, `<div key={d._id} onClick={() => setSelectedDriverForEdit(d)} className="cursor-pointer bg-white border`);
content = content.replace(/<div key={v\._id} className="min-w-\[300px\] snap-center shrink-0/g, `<div key={v._id} onClick={() => setSelectedVehicleForEdit(v)} className="cursor-pointer min-w-[300px] snap-center shrink-0`);
