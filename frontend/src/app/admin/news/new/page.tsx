...
const createNews = async (values: z.infer<typeof formSchema>, token: string) => {
...
export default function CreateNews() {
  ...
  const mutation = useMutation({
    mutationFn: (values: z.infer<typeof formSchema>) => createNews(values, token),
    onSuccess: () => {
...
<div className="space-y-2">
  ...
  />
      ...
      {mutation.isSuccess && <p className="text-green-500 text-sm">News created successfully!</p>}
    </div>
  )
}
