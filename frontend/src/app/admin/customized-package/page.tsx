...
const getAllPackages = async (accessToken: string) => {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL}/customizedpackage/?status=`,
    {
      method: "GET",
      headers: {
...
function PackagePage() {
  ...
  const handleEdit = (id: string) => {
    router.push(`/admin/customized-package/edit/${id}`);
  };
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold">All Customized Packages</h1>
        <button
          onClick={() => router.push("/admin/customized-package/new")}
