...
function UpdateFooter() {
    ...
    const ContentArray = useFieldArray({ control: form.control, name: "content" })
  ...
  return (
    <div className="flex  items-center justify-center bg-gray-50 px-4">
      <div className="w-full max-w-5xl rounded-2xl bg-white p-6 shadow-md">
        <h1 className="mb-6 text-center text-2xl font-bold">Update a Footer Item</h1>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div>
              ...
              {ContentArray.fields.map((field, index) => (
                <div key={field.id} className="grid grid-cols-2 gap-2 mb-2">
                  <Input {...form.register(`content.${index}.text`)} placeholder="Text" />
                  <Input {...form.register(`content.${index}.url`)} placeholder="Url" />
                  <Button
                    type="button"
                    variant="destructive"
            ...
            <Button type="button" onClick={() => ContentArray.append({ text: "", url: "" })}>
                Add Item
            </Button>
                </div>
             ...
             {mutation.isSuccess && <p className="text-green-500">Item Updated successfully!</p>}
           </div>
         </div>
    )
}
