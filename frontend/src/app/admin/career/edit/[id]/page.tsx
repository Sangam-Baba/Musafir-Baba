...
async function updateJob(values: Job, accessToken: string, id: string) {
...
async function getJob(id: string) {
  ...
  if (!res.ok) {
    const errorData = await res.json();
    throw new Error(errorData.message || "Job getting failed");
  }

  return res.json();
}
export default function CreateJob({ params }: { params: { id: string } }) {
  ...
  const mutation = useMutation({
    mutationFn: (values: Job) => updateJob(values, accessToken, id),
    onSuccess: () => {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gray-50 px-4">
      <div className="w-full max-w-2xl rounded-2xl bg-white p-6 shadow-md">
        <h1 className="mb-6 text-center text-2xl font-bold">Edit Job</h1>
        ...
        {mutation.isSuccess && (
          <p className="text-green-500"> {"Job Updated successfully"} </p>
        )}
      </div>
    </div>
  );
