...
const update=async(values:z.infer<typeof formSchema>, token:string, slug:string )=>{
...
const getCategory = async (id:string) => {
    const res=await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/category/id/${id}`,{
        method:"GET",
        headers:{"Content-Type":"application/json"},
        credentials:"include",
    })
...
export default function CreateCategory() {
  ...
  const mutation = useMutation({
    mutationFn: (values: z.infer<typeof formSchema>) => update(values, accessToken, category?.slug || ""),
    onSuccess: () => {
      form.reset();
  ...
  if( isLoading || isLoadingCategory) return <Loader size="lg" message="Loading..." />
  if(isError) return <p>Error: {error?.message}</p>
  if( isErrorCategory) return <p>Error: {errorCategory?.message}</p>
