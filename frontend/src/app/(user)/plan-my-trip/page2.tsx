...
import CustomTourStepper from "./page";
...
export default function CustomTourDialog() {
  ...
  useEffect(() => {
    ...
    chosenPackage,
    ...watchedValues, // useWatch gives stable dependencies
  ]);
  ...
  const onSubmit = (data: FormData) => {
    console.log("FINAL BOOKING PAYLOAD:", data);
    // frontend-only preview requested — no POST
    close();
  };

  /* -------------------- Render Step UI -------------------- */
  // Step 1: package, people, city
  const Step1 = (
    <>
      <FormField
        ...
        <div className="mt-3">
          ...
          <div className="grid grid-cols-1 gap-2 mt-2">
            {chosenPackage?.city?.map((act) => {
              const checked = !!watch("city")?.find((a) => a.name === act.name);
              return (
                <label key={act.name} className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    checked={checked}
                    onChange={(e) => {
  ...
  const Step2 = (
    <>
      ...
      <select
        ...
        onChange={(e) =>
          setValue("duration.type", e.target.value as "fixed" | "flexible")
        }
      >
      ...
      {watch("duration.type") === "fixed" && (
        <div className="flex gap-2 mb-2">
          <label className="flex-1">
            <div className="text-sm">Start Date</div>
            <Input type="date" {...register("duration.fixed.startDate")} />
          </label>
          <label className="flex-1">
            <div className="text-sm">End Date</div>
            <Input type="date" {...register("duration.fixed.endDate")} />
          </label>
        </div>
      )}
      {watch("duration.type") === "flexible" && (
        <div className="mb-4">
  ...
  // Step 5 summary is built inline below
  ...
  return (
    <>
      ...
      <Dialog
        ...
        onOpenChange={(value) => {
          if (!value) {
            // only close if user clicks backdrop or presses ESC
            setOpen(false);
            setTimeout(() => {
              reset();
              setCurrentStep(0);
                    ...
                    {currentStep === 4 && (
                      <>
                        ...
                        <div className="text-sm text-gray-700">
                          <p>
                            Destination: {chosenPackage?.destination?.state}
                          </p>
                          <p>
                            Place to visit:{" "}
                            {watch("city")
                          ...
                          <p>
                            ...
                            {watch("duration.fixed")?.startDate
                              ? `${watch("duration.fixed")?.startDate} → ${
                                  ...
                                  watch("duration.fixed")?.totalDays || 1
                                } days)`
                        ...
                        <p className="text-right font-semibold mt-1">
                          @GST: 5% — ₹
                          {((watch("finalPrice") || 0) * 0.05).toFixed(2)}
                        </p>
