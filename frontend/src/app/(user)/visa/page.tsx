...
const getVisa= async(search:string)=>{
    const res= await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/visa/?country=${search}`);
...
function VisaPage() {
 ...
 const [loading, setLoading] = React.useState(false);
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const { data: visa, isLoading, isError, error , refetch} = useQuery({
    ...
    <section>
        <div className='relative'>
           <Hero image="/Heroimg.jpg" title="Visa" />
            <form onSubmit={handleSubmit} className=' z-10 absolute bottom-0 left-1/2 transform -translate-x-1/2 -translate-y-1/2 flex border border-gray-300 rounded-md overflow-hidden '>
                <Input className='border-none text-white' type="text" placeholder="Search for a country" value={search} onChange={handleChange} />
                <Button className='border-none bg-[#FF5300] hover:bg-[#FE5300]' type="submit">Search</Button>
            </form>
        </div>
