...
async function getBooking(bookingId: string, accessToken: string | null) {
  ...
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL}/customizedtourbooking/${bookingId}`,
    {
      method: "GET",
      headers: {
...
export default function CheckoutButton({ params }: { params: { id: string } }) {
  ...
  const bookingId = params.id; // safe conversion
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/payment`, {
        ...
        body: JSON.stringify({
          ...
          productinfo: booking?.packageId?.title ?? "Customized Travel Package",
          firstname: booking?.userId?.name ?? "Guest",
          email: booking?.userId?.email ?? "abhi@example.com",
          phone: "9876543210",
          udf1: params.id,
          surl: `${process.env.NEXT_PUBLIC_BASE_URL}/payment/success-customized-tour`,
          furl: `${process.env.NEXT_PUBLIC_BASE_URL}/payment/failure-customized-tour`,
        ...
        <div className="lg:col-span-2">
          <Card className="overflow-hidden border-0 shadow-xl hover:shadow-2xl transition-shadow duration-300 p-0">
            <div className="relative h-80 md:h-96 overflow-hidden bg-muted">
              <Image
                src={booking?.packageId?.coverImage?.url ?? ""}
                  ...
                  <div className="space-y-2">
                    ...
                    <div>
                      ...
                      <p className="font-medium text-foreground text-sm break-all">
                        {booking?.userId?.email ?? "abhi@example.com"}
                      </p>
                    </div>
                  </div>
                </div>
