                ...
                {records.map((record) => (
                  <TableRow key={record._id} className="hover:bg-slate-50/80 transition-all duration-300 group cursor-default border-slate-100">
                    ...
                    <TableCell className="py-2 text-[13px] font-semibold text-slate-600">
                      {record.totalOfficeHours ? record.totalOfficeHours.toFixed(2) : (record.checkInTime && !record.checkOutTime && record.date.startsWith(new Date().toISOString().split("T")[0]) ? (
                    ...
                    <TableCell className="py-2 text-[13px] font-bold text-[#FE5300]">
                      <div className="flex items-center gap-2">
                        {record.totalWorkingHours ? record.totalWorkingHours.toFixed(2) : (record.checkInTime && !record.checkOutTime && record.date.startsWith(new Date().toISOString().split("T")[0]) ? (
