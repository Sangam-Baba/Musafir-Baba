...
import SlugClients from "@/app/(user)/holidays/[categorySlug]/[destination]/[packageSlug]/SlugClients";
import { useAdminAuthStore } from "@/store/useAdminAuthStore";
import { Loader } from "@/components/custom/loader";

export default function PreviewDrawer({ id, isOpen, onClose }: { id: string | null; isOpen: boolean; onClose: () => void }) {
    ...
    <Sheet open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <SheetContent side="right" className="w-[90vw] sm:max-w-[90vw] overflow-y-auto p-0 bg-slate-50">
        <SheetHeader className="p-4 bg-white border-b sticky top-0 z-50">
          <SheetTitle>Package Preview {pkg?.pendingUpdates ? "(Showing Pending Updates)" : ""}</SheetTitle>
        </SheetHeader>
        {loading ? (
          <div className="p-8">
          ...
          <div className="w-full relative z-0">
            <SlugClients pkg={pkg} relatedGroupPackages={[]} />
          </div>
        ) : (
