            ...
            <View className="p-4 space-y-4 animate-in fade-in duration-200">
                ...
                <View className="flex items-center justify-between pt-1 flex-row">
                  <View className="flex items-center gap-3 flex-row flex-1 min-w-0">
                    ...
                    <View className="flex-1 min-w-0">
                      <View className=""><Text className="text-xs font-black text-slate-900">{selectedOffer?.category || 'Vehicle'}</Text></View>
                      <View className=""><Text className="text-[10px] text-slate-500 font-bold">{selectedOffer?.vehicleName || ''}</Text></View>
                      <View className="flex items-center gap-2 pt-1 flex-row flex-wrap">
                        <Text className="flex items-center gap-0.5"><Users className="w-3 h-3 text-slate-400"/> {selectedOffer?.seatingCapacity || '-'} Passengers</Text>
                        <Text className="flex items-center gap-0.5 text-emerald-600"><Snowflake className="w-3 h-3"/> AC Vehicle</Text>
                      </View>
                    </View>
                  </View>

                  <TouchableOpacity onPress={() => onNavigate('vehicle-select')} className="flex items-center gap-0.5 flex-row shrink-0 ml-2">
                ...
                <View className="space-y-2.5">
                  <View className="flex justify-between items-center flex-row">
                    <Text className="flex items-center gap-2 text-slate-600">
                      ...
                      Base Fare ({quote?.distanceKm ?? 0} km)
                    </Text>
                    <Text className="font-black text-slate-900">{(selectedOffer?.baseFare ?? 0).toLocaleString('en-IN')}</Text>
                  </View>
                  ...
                  <View className="flex justify-between items-center flex-row">
                    ...
                    <Text className="font-black text-slate-900">{(selectedOffer?.driverAllowance ?? 0).toLocaleString('en-IN')}</Text>
                  </View>
                  ...
                  <View className="pt-2.5 border-t border-slate-100 flex justify-between items-center flex-row">
                    ...
                    <View className=""><Text className="text-lg font-black text-slate-900">₹{(selectedOffer?.totalAmount ?? 0).toLocaleString('en-IN')}</Text></View>
                  </View>
                </View>
              </View>
              ...
              <View className="bg-white border border-slate-200/80 rounded-3xl p-4 space-y-3 shadow-md">
                <View className="flex justify-between items-center flex-row">
                  <View className="flex items-center gap-2.5 flex-row">
                    ...
                    <View>
                      ...
                      <View className=""><Text className="text-base font-black text-slate-900">₹{(selectedOffer?.totalAmount ?? 0).toLocaleString('en-IN')}</Text></View>
                    </View>
                  </View>
                ...
                >
                  ...
                  <Text className="text-slate-700 font-medium">{isBooking ? 'Booking...' : 'Proceed to Booking'}</Text>
                </TouchableOpacity>
              </View>
