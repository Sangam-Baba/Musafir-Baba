...
const createVisa= async(req, res)=>{
    try {
        const {country, duration, cost, visaType}= req.body;
        if(!country || !duration || !cost || !visaType ){
...
const updateVisa = async (req, res)=>{
    try{
     ...
     res.status(200).json({success:true,message: "Update successful", data:visa} );
    }
    catch(error){
...
const deleteVisa = async( req, res)=>{
    try {
        ...
        if(!visa) return res.status(404).json({success:false, message:"Invalid Id"});
        res.status(200).json({success:true,message: "Visa Delete successful", data:visa} );
    } catch (error) {
