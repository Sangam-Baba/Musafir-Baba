...
const getWebPageBySlug= async (req, res)=>{
    try {
        ...
        if(!slug) return res.status(404).json({success:false, message:"Invalid Slug"});
        const webpage= await WebPage.findOne({slug}).lean();
        if(!webpage) return res.status(404).json({success:false, message:"Invalid Slug"});
        res.status(200).json({success:true, data:webpage});
    } catch (error) {
...
const  getWebPageById= async (req, res)=>{
    try {
        const { id } = req.params;
        console.log(id);
        if(!id) return res.status(404).json({success:false, message:"Invalid id"});
        const webpage= await WebPage.findById({_id:id}).lean();
        if(!webpage) return res.status(404).json({success:false, message:"Invalid id"});
        res.status(200).json({success:true, data:webpage});
    } catch (error) {
...
const updateWebPage =async (req, res)=>{
    try {
        ...
        const webpage= await WebPage.findByIdAndUpdate(id, req.body, {new:true});
        if(!webpage){
            return res.status(404).json({success:false, message: "WebPage not found"});
        }
        res.status(200).json({success:true, message:"WebPage updated successfully", data:webpage});
    } catch (error) {
...
const deleteWebPage =async (req, res)=>{
    try {
        ...
        res.status(200).json({success:true, message:"WebPage deleted successfully", data:webpage});
    } catch (error) {
        console.log("WebPage delete failed", error.message);
        res.status(500).json({success: false, message: "Server Error"});
    }
}
export {createWebPage , getWebPage, getWebPageBySlug , updateWebPage, deleteWebPage , getWebPageById};