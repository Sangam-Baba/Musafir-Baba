...
return res.status(201).json({ success: true, message:"Booking Success", data: created });
...
const getMyBookings=async(req, res)=>{
    try {
      ...
      return res.json({ success: true, meta: { total, page, limit }, data: bookings, message:"Fetching data successfull"});
    } catch (error) {
...
if (req.query.PaymentStatus) filter.PaymentStatus = req.query.PaymentStatus;
    ...
    return res.json({ success: true, meta: { total, page, limit }, data: bookings , message:"Fetching all bookings" });
    } catch (error) {
...
const cancelMyBooking=async(req, res)=>{
   try {
      ...
      if(!booking) return res.status(404).json({success:false, message:"Booking not found"});
      if(String(booking.user) !== String(userId))  return res.status(403).json({ success: false, error: "Forbidden" });
      if(booking.bookingStatus ==="Cancelled") return res.status(400).json({ success: false, error: "Already cancelled" });
      ...
      await booking.save(); 
      return res.json({ success: true, data: booking , message:"Booking Canceled"});
   } catch (error) {
...
export {createBooking , getMyBookings , getAllBookings , getBookingById,adminUpdateBookingStatus, cancelMyBooking }