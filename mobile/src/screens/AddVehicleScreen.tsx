...
export const AddVehicleScreen = () => {
  ...
  // Tab 1 State: 6 Vehicle Inspection Images
  const [frontImage, setFrontImage] = useState(false);
  const [backImage, setBackImage] = useState(false);
  const [leftSideImage, setLeftSideImage] = useState(false);
  const [rightSideImage, setRightSideImage] = useState(false);
  const [interiorFrontImage, setInteriorFrontImage] = useState(false);
  const [interiorBackImage, setInteriorBackImage] = useState(false);

  // Tab 2 State: Vehicle Documents
  const [rcUploaded, setRcUploaded] = useState(false);
  const [insuranceUploaded, setInsuranceUploaded] = useState(false);
  const [pucUploaded, setPucUploaded] = useState(false);
  ...
  const handleNextStep1 = () => {
    ...
    if (!frontImage || !backImage || !leftSideImage || !rightSideImage || !interiorFrontImage || !interiorBackImage) {
      Alert.alert("Missing Photos", "Please upload all 6 required vehicle photos (1 Front, 1 Back, 2 Sides, 2 Interior).");
      return;
    }
    setCurrentStep(2);
  };
  ...
  const handleSubmitVehicle = async () => {
    if (!driverName || !driverMobile || !driverLicense) {
      Alert.alert("Missing Driver Info", "Please fill in all Driver Details (Name, Mobile, License).");
      return;
    }
      ...
      const res = await fetch(`${API_BASE_URL}/partner/vehicle`, {
        ...
        body: JSON.stringify({
          vehicleData: {
            ...
            color: color,
            rcImageUrl: "https://storage.musafirbaba.com/docs/rc.pdf",
            insuranceFileUrl: "https://storage.musafirbaba.com/docs/insurance.pdf",
            pucImageUrl: "https://storage.musafirbaba.com/docs/puc.pdf",
            permitFileUrl: "https://storage.musafirbaba.com/docs/permit.pdf",
            frontImageUrl: "https://storage.musafirbaba.com/photos/front.jpg",
            rearImageUrl: "https://storage.musafirbaba.com/photos/back.jpg",
            leftSideImageUrl: "https://storage.musafirbaba.com/photos/left.jpg",
            rightSideImageUrl: "https://storage.musafirbaba.com/photos/right.jpg",
            interiorImageUrl: "https://storage.musafirbaba.com/photos/interior.jpg",
            otherImageUrl: "https://storage.musafirbaba.com/photos/interior_back.jpg",
            driver: {
              name: driverName.trim(),
              mobile: driverMobile.trim(),
              licenseNo: driverLicense.trim(),
              licenceImageUrl: "https://storage.musafirbaba.com/docs/licence.jpg"
            }
          }
        })
      });
              ...
              <View style={styles.photoUploadItem}>
                <View style={styles.photoLeftInfo}>
                  ...
                  <View style={{ marginLeft: 10 }}>
                    ...
                    <Text style={styles.photoSub}>{frontImage ? 'front_view.jpg (Uploaded)' : 'Clear front view with reg plate'}</Text>
                  </View>
                </View>
                <TouchableOpacity 
                  style={[styles.uploadPillBtn, frontImage && styles.uploadPillBtnSuccess]} 
                  onPress={() => setFrontImage(!frontImage)}
                >
                  <Ionicons name={frontImage ? "checkmark-circle" : "cloud-upload-outline"} size={14} color={frontImage ? "#16a34a" : "#FE5300"} />
                  <Text style={[styles.uploadPillText, frontImage && { color: "#16a34a" }]}>{frontImage ? "Uploaded" : "Capture"}</Text>
                </TouchableOpacity>
              </View>
              ...
              <View style={styles.photoUploadItem}>
                <View style={styles.photoLeftInfo}>
                  ...
                  <View style={{ marginLeft: 10 }}>
                    ...
                    <Text style={styles.photoSub}>{backImage ? 'rear_view.jpg (Uploaded)' : 'Clear rear view showing rear reg plate'}</Text>
                  </View>
                </View>
                <TouchableOpacity 
                  style={[styles.uploadPillBtn, backImage && styles.uploadPillBtnSuccess]} 
                  onPress={() => setBackImage(!backImage)}
                >
                  <Ionicons name={backImage ? "checkmark-circle" : "cloud-upload-outline"} size={14} color={backImage ? "#16a34a" : "#FE5300"} />
                  <Text style={[styles.uploadPillText, backImage && { color: "#16a34a" }]}>{backImage ? "Uploaded" : "Capture"}</Text>
                </TouchableOpacity>
              </View>
              ...
              <View style={styles.photoUploadItem}>
                <View style={styles.photoLeftInfo}>
                  ...
                  <View style={{ marginLeft: 10 }}>
                    ...
                    <Text style={styles.photoSub}>{leftSideImage ? 'left_side.jpg (Uploaded)' : 'Full left exterior side profile'}</Text>
                  </View>
                </View>
                <TouchableOpacity 
                  style={[styles.uploadPillBtn, leftSideImage && styles.uploadPillBtnSuccess]} 
                  onPress={() => setLeftSideImage(!leftSideImage)}
                >
                  <Ionicons name={leftSideImage ? "checkmark-circle" : "cloud-upload-outline"} size={14} color={leftSideImage ? "#16a34a" : "#FE5300"} />
                  <Text style={[styles.uploadPillText, leftSideImage && { color: "#16a34a" }]}>{leftSideImage ? "Uploaded" : "Capture"}</Text>
                </TouchableOpacity>
              </View>
              ...
              <View style={styles.photoUploadItem}>
                <View style={styles.photoLeftInfo}>
                  ...
                  <View style={{ marginLeft: 10 }}>
                    ...
                    <Text style={styles.photoSub}>{rightSideImage ? 'right_side.jpg (Uploaded)' : 'Full right exterior side profile'}</Text>
                  </View>
                </View>
                <TouchableOpacity 
                  style={[styles.uploadPillBtn, rightSideImage && styles.uploadPillBtnSuccess]} 
                  onPress={() => setRightSideImage(!rightSideImage)}
                >
                  <Ionicons name={rightSideImage ? "checkmark-circle" : "cloud-upload-outline"} size={14} color={rightSideImage ? "#16a34a" : "#FE5300"} />
                  <Text style={[styles.uploadPillText, rightSideImage && { color: "#16a34a" }]}>{rightSideImage ? "Uploaded" : "Capture"}</Text>
                </TouchableOpacity>
              </View>
              ...
              <View style={styles.photoUploadItem}>
                <View style={styles.photoLeftInfo}>
                  ...
                  <View style={{ marginLeft: 10 }}>
                    ...
                    <Text style={styles.photoSub}>{interiorFrontImage ? 'dashboard.jpg (Uploaded)' : 'Front dashboard & steering wheel'}</Text>
                  </View>
                </View>
                <TouchableOpacity 
                  style={[styles.uploadPillBtn, interiorFrontImage && styles.uploadPillBtnSuccess]} 
                  onPress={() => setInteriorFrontImage(!interiorFrontImage)}
                >
                  <Ionicons name={interiorFrontImage ? "checkmark-circle" : "cloud-upload-outline"} size={14} color={interiorFrontImage ? "#16a34a" : "#FE5300"} />
                  <Text style={[styles.uploadPillText, interiorFrontImage && { color: "#16a34a" }]}>{interiorFrontImage ? "Uploaded" : "Capture"}</Text>
                </TouchableOpacity>
              </View>
              ...
              <View style={[styles.photoUploadItem, { borderBottomWidth: 0 }]}>
                <View style={styles.photoLeftInfo}>
                  ...
                  <View style={{ marginLeft: 10 }}>
                    ...
                    <Text style={styles.photoSub}>{interiorBackImage ? 'passenger_seats.jpg (Uploaded)' : 'Clean passenger seating area'}</Text>
                  </View>
                </View>
                <TouchableOpacity 
                  style={[styles.uploadPillBtn, interiorBackImage && styles.uploadPillBtnSuccess]} 
                  onPress={() => setInteriorBackImage(!interiorBackImage)}
                >
                  <Ionicons name={interiorBackImage ? "checkmark-circle" : "cloud-upload-outline"} size={14} color={interiorBackImage ? "#16a34a" : "#FE5300"} />
                  <Text style={[styles.uploadPillText, interiorBackImage && { color: "#16a34a" }]}>{interiorBackImage ? "Uploaded" : "Capture"}</Text>
                </TouchableOpacity>
              </View>
            </View>
            ...
            <View style={styles.formCard}>
              ...
              <View style={styles.docUploadRow}>
                ...
                <View style={{ flex: 1, marginLeft: 10 }}>
                  ...
                  <Text style={styles.docSub}>{rcUploaded ? 'rc_certificate.pdf (Uploaded)' : 'Upload clear photo/scan of Vehicle RC'}</Text>
                </View>
                <TouchableOpacity 
                  style={[styles.uploadBtn, rcUploaded && styles.uploadBtnSuccess]} 
                  onPress={() => setRcUploaded(!rcUploaded)}
                >
                  <Ionicons name={rcUploaded ? "checkmark-circle" : "cloud-upload-outline"} size={16} color={rcUploaded ? "#16a34a" : "#FE5300"} />
                  <Text style={[styles.uploadBtnText, rcUploaded && { color: "#16a34a" }]}>{rcUploaded ? "Uploaded" : "Upload"}</Text>
                </TouchableOpacity>
              </View>
              ...
              <View style={styles.docUploadRow}>
                ...
                <View style={{ flex: 1, marginLeft: 10 }}>
                  ...
                  <Text style={styles.docSub}>{insuranceUploaded ? 'insurance_policy.pdf (Uploaded)' : 'Valid comprehensive commercial policy'}</Text>
                </View>
                <TouchableOpacity 
                  style={[styles.uploadBtn, insuranceUploaded && styles.uploadBtnSuccess]} 
                  onPress={() => setInsuranceUploaded(!insuranceUploaded)}
                >
                  <Ionicons name={insuranceUploaded ? "checkmark-circle" : "cloud-upload-outline"} size={16} color={insuranceUploaded ? "#16a34a" : "#FE5300"} />
                  <Text style={[styles.uploadBtnText, insuranceUploaded && { color: "#16a34a" }]}>{insuranceUploaded ? "Uploaded" : "Upload"}</Text>
                </TouchableOpacity>
              </View>
              ...
              <View style={styles.docUploadRow}>
                ...
                <View style={{ flex: 1, marginLeft: 10 }}>
                  ...
                  <Text style={styles.docSub}>{pucUploaded ? 'puc_certificate.jpg (Uploaded)' : 'Pollution under control certificate'}</Text>
                </View>
                <TouchableOpacity 
                  style={[styles.uploadBtn, pucUploaded && styles.uploadBtnSuccess]} 
                  onPress={() => setPucUploaded(!pucUploaded)}
                >
                  <Ionicons name={pucUploaded ? "checkmark-circle" : "cloud-upload-outline"} size={16} color={pucUploaded ? "#16a34a" : "#FE5300"} />
                  <Text style={[styles.uploadBtnText, pucUploaded && { color: "#16a34a" }]}>{pucUploaded ? "Uploaded" : "Upload"}</Text>
                </TouchableOpacity>
              </View>
              ...
              <View style={[styles.docUploadRow, { borderBottomWidth: 0 }]}>
                ...
                <View style={{ flex: 1, marginLeft: 10 }}>
                  ...
                  <Text style={styles.docSub}>{permitUploaded ? 'rto_permit.pdf (Uploaded)' : 'All India / State Tourist Permit'}</Text>
                </View>
                <TouchableOpacity 
                  style={[styles.uploadBtn, permitUploaded && styles.uploadBtnSuccess]} 
                  onPress={() => setPermitUploaded(!permitUploaded)}
                >
                  <Ionicons name={permitUploaded ? "checkmark-circle" : "cloud-upload-outline"} size={16} color={permitUploaded ? "#16a34a" : "#FE5300"} />
                  <Text style={[styles.uploadBtnText, permitUploaded && { color: "#16a34a" }]}>{permitUploaded ? "Uploaded" : "Upload"}</Text>
                </TouchableOpacity>
              </View>
            </View>
