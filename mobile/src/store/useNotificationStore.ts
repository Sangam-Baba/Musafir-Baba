...
import { getMyNotifications, markAllNotificationsRead, PartnerNotificationData } from '../api/partnerNotification.api';

interface RealtimeNotificationEvent {
  id: string;
  title: string;
  message: string;
  type: PartnerNotificationData['type'];
  data?: Record<string, unknown>;
  createdAt?: string;
}

interface NotificationState {
  notifications: PartnerNotificationData[];
  unreadCount: number;
  isLoading: boolean;
...
export const useNotificationStore = create<NotificationState>((set, get) => ({
  ...
  handleRealtimeEvent: (event) => {
    const incoming: PartnerNotificationData = {
      id: event.id,
      title: event.title,
